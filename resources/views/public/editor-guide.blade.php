...
@section('content')
    <div class="card mx-auto mb-6 max-w-3xl">
        <div class="card-body sm:p-8">
            ...
            <p class="mt-3 text-slate-700 dark:text-slate-200">{{ __('Deux choses à savoir avant de commencer. Écrire demande un compte contributeur, c\'est-à-dire une preuve que vous avez réellement contribué à l\'écosystème : lire et s\'abonner n\'en demandent pas. Et toute annonce passe par la revue : un jeton d\'API donne le droit de soumettre, jamais celui de publier. Seules ses traductions y échappent, le texte qu\'elles traduisent ayant déjà été relu.') }}</p>
...
curl -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    ...
    <div class="card mx-auto mb-6 max-w-3xl">
        <div class="card-body sm:p-8">
            ...
            <p class="mt-3 text-slate-700 dark:text-slate-200">{{ __('Une fois publié, un article ne se modifie plus en place : vous proposez une révision, qui repasse par la revue, l\'article porte la mention de sa correction et la version d\'origine reste consultable. Une traduction est un article à part entière, liée à son original : celle que vous déposez vous-même paraît sans revue, celle d\'un traducteur que vous avez mandaté passe devant un relecteur ; une annonce non traduite n\'est jamais désavantagée dans la diffusion.') }}</p>
